    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            ZStack(alignment: .trailing) {
                Group {
                    if showCurrentPassword {
                        TextField("password_current", text: $currentPassword)
                    } else {
                        SecureField("password_current", text: $currentPassword)
                    }
                }
            ...
            ZStack(alignment: .trailing) {
                Group {
                    if showNewPassword {
                        TextField("password_new", text: $newPassword)
                    } else {
                        SecureField("password_new", text: $newPassword)
                    }
                }
                .textContentType(.password)
                .autocapitalization(.none)
            ...
            ZStack(alignment: .trailing) {
                Group {
                    if showConfirmNewPassword {
                        TextField("password_new_confirm", text: $ConfirmNewPassword)
                    } else {
                        SecureField("password_new_confirm", text: $ConfirmNewPassword)
                    }
                }
                .textContentType(.password)
                .autocapitalization(.none)
